...
const greedySearch: ISearchFunction = (playablePieces: Array<IPlayablePiece>) => {
  ...
  const fn = (position: number): IPlayablePiece => {
    ...
    if (piecePrice(playablePieces?.[position - 1]?.piece || [-1,-1]) > actualPiecePrice) {
      return fn(--position)
    }
...
const mountGraph = (maxDeep: number) => (searchFn: ISearchFunction) => (player1Pieces: Array<Domino>) => (player2Pieces: Array<Domino>) => (edges: Array<Edge>): Array<IGraph | null> => {
  let actualDeep = 0
  ...
  const mountNodeTree = (prevPieceSide: Edge) => (actualPieces: Array<Domino>) => (nextPieces: Array<Domino>): IGraph | null => {
    ...
    const notPlayableSide: Edge = {
      value: bestPiece.piece[0] === prevPieceSide.value ? bestPiece.piece[1] : bestPiece.piece[0],
